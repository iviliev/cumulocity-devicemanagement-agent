#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""
Copyright (c) 2022 Software AG, Darmstadt, Germany and/or its licensors

SPDX-License-Identifier: Apache-2.0

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

        http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
"""
import logging
import io
import re
from posixpath import dirname
from datetime import datetime
from c8ydm.framework.modulebase import Initializer, Listener
from c8ydm.framework.smartrest import SmartRESTMessage
from os.path import expanduser, exists, dirname
import pathlib
import subprocess


class SendCertificateInitializer(Initializer, Listener):
    xid = 'c8y-dm-agent-v1.0'
    logger = logging.getLogger(__name__)
    fragment = 'c8y_SendCertificate'

    def getMessages(self):
        return []

    def getSupportedOperations(self):
        return ['c8y_SendCertificate']

    def getSupportedTemplates(self):
        return [self.xid]

    def _set_executing(self):
        executing = SmartRESTMessage('s/us', '501', [self.fragment])
        self.agent.publishMessage(executing)

    # datei anhängen
    def _set_success(self, url):
        success = SmartRESTMessage('s/us', '503', [self.fragment, url])
        self.agent.publishMessage(success)

    def _set_failed(self, reason):
        failed = SmartRESTMessage('s/us', '502', [self.fragment, reason])
        self.agent.publishMessage(failed)

    def handleOperation(self, message):
        home = expanduser('~')
        root = pathlib.Path(home + '/.cumulocity')
        self.logger.info(root)
        configfiles = {'agent': f'{root}/agent.ini'}
        try:
            self.logger.info(message.messageId)
            if 's/ds' in message.topic:
                binaryurl = message.values[0]
                certificate_file_name = self.agent.client_cert
                self.logger.info(certificate_file_name)
                self._set_executing()
                self.logger.info(binaryurl)
                if 'cumulocity' in binaryurl:
                    path = pathlib.Path(
                        certificate_file_name)
                    self.logger.info(path)
                    self.logger.info('Start downloading the certificate file')
                    self.logger.info(dirname(path))
                    # if pathlib.Path(path).exists():
                    #     process = subprocess.Popen(["cp", str(
                    #         path), f'{str(path)}_backup'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                    #     process.wait()
                    if self.agent.rest_client.download_c8y_binary(binaryurl, str(path)) is not None:
                        eventMsg = SmartRESTMessage(
                            's/us', '400', ['c8y_SendCertificateEvent', f'Certificate file was downloaded to {str(path)}.'])
                        self.agent.publishMessage(eventMsg)
                        self._set_success(binaryurl)
                    else:
                        self._set_failed(
                            "Failed to download certificate file from c8y binary")
                    # else:
                    #     self._set_failed(
                    #         "Directory of config file does not exist")
                # else:
                #     self._set_failed("Do not know config file type")
                else:
                    self._set_failed("Currently only c8y binary supported")
            self.logger.debug("send certificate handled")
        except Exception as e:
            self._set_failed(e)
